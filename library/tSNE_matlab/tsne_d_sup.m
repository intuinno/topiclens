function ydata = tsne_d_sup(DD, labels, shrink_factor, add_val, no_dims, initial_dims, perplexity)
...
shrink_factor
    ...
    for i=min(labels):max(labels)
        for j=min(labels):max(labels)
            ...
            DD(labels==i,labels==j) = DD(labels==i,labels==j) + add_val;
        end
    end

    [sorted_label,idx] = sort(labels);
    figure();imagesc(DD(idx,idx));colorbar;
    figure();

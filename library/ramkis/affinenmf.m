function [W, H, w1, iter] = affinenmf(A, Winit, Hinit, params)
...
for iter = 1 : maxiter
	...
	left = W' * W;
	right = A' * W;
	H = anls_entry_blockpivot_precompute(left, right-temp(:,ones(1,n))', H')';
	Hlarge = [h1; H];
	gradH = left*H-right'+temp(:,ones(1,n));
	left = Hlarge * Hlarge';
	right = A * Hlarge';
	gradWlarge = Wlarge*left-right;
	if iter == 1
		initgrad = sqrt(norm(gradWlarge(gradWlarge<=0|Wlarge>0))^2 + norm(gradH(gradH<=0|H>0))^2);
		continue;
	else
		projnorm = sqrt(norm(gradWlarge(gradWlarge<=0|Wlarge>0))^2 + norm(gradH(gradH<=0|H>0))^2);
	end
